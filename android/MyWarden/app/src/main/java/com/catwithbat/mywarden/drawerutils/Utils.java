package com.catwithbat.mywarden.drawerutils;
...
import com.catwithbat.mywarden.fragments.FragmentSchedule;
import com.catwithbat.mywarden.fragments.FragmentStatistics;
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id, IDrawerItem drawerItem) {
                ...
                if (drawerItem != null) {
                    switch (drawerItem.getIdentifier()){
                        case 1:  activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new FragmentWorkDay(), FragmentWorkDay.TAG).commit(); break;
                        case 2:  activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new FragmentStatistics(), FragmentStatistics.TAG).commit(); break;
                        case 3:  activity.getSupportFragmentManager().beginTransaction().replace(R.id.frame_container, new FragmentSchedule(), FragmentSchedule.TAG).commit(); break;
                    }
                }
            }
        };
...
*/
    ...
    public static Drawer.Result createCommonDrawer(final ActionBarActivity activity, Toolbar toolbar, AccountHeader.Result headerResult) {
        ...
        Drawer.Result drawerResult = new Drawer()
                ...
                .addDrawerItems(
                        new PrimaryDrawerItem().withName(R.string.frag_work_name).withIcon(GoogleMaterial.Icon.gmd_work).withIdentifier(1),
                        new PrimaryDrawerItem().withName(R.string.frag_stat_name).withIcon(GoogleMaterial.Icon.gmd_trending_up).withIdentifier(2),
                        // Вывод НЕ для Google Play
                        new PrimaryDrawerItem().withName(R.string.frag_sch_name).withIcon(GoogleMaterial.Icon.gmd_toc).withIdentifier(3),
